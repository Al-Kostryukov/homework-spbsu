...
class Decoder(path : String) {
  ...
  private def head (bitVector: BitVector): GIFSizes = {
    try {
      ...
      val headerCodec = (constant(hex"474946383961".bits) :: int16L :: uint16L).as[GIFSizes] //GIF89a

      val decoded = headerCodec.toCodec.decode(bitVector)
  ...
  private def headsplit(bitvec: BitVector): GifHead = {
    ...
    if (bitv(0) == false) {
      ...
      return  GifHead(Sizes, BG, PalitraParametrs(0,0), null)
    }
    else {
  ...
  private def bodyDecode(gifH: GifHead, v: BitVector, GraphContrList: List[GraphicControl], TextList: List[PlainText], ImList: List[Images]): GifBody = {
    ...
    if (hex"3b".bits == v.take(8)) GifBody(GraphContrList, TextList, ImList)
      ...
      val decoded = par.toCodec.decode(vec)
      vec = vec.drop(16 * 4)
      if (vec.take(1) == true) {
        ...
        if (pict.v.length % (254 * 8) == 0)
        	bodyDecode(gifH, vec.drop(16 + 8 + 8 * 3 * loc.size + pict.v.length + 8 * (pict.v.length / (8 * 254))), GraphContrList, TextList, pict :: ImList)
        else
        	bodyDecode(gifH,vec.drop(16 + 8 + 8 * 3 * loc.size + pict.v.length + 8 * (pict.v.length / (8 * 254)) + 8), GraphContrList, TextList, pict :: ImList)
        ...
        if (pict.v.length % (254 * 8) == 0)
        	bodyDecode(gifH, vec.drop(16+8 + pict.v.length + 8 * (pict.v.length.toInt / (8 * 254))), GraphContrList, TextList,  pict :: ImList)
        else
        	bodyDecode(gifH,vec.drop(16+8 + pict.v.length + 8 * (pict.v.length.toInt / (8 * 254)) + 8), GraphContrList, TextList, pict :: ImList)
      }
    }
    else{
      ...
      if (hex"f9".bits == v.drop(8).take(8) ){
        ...
        val decoded = grap.toCodec.decode(vec)
      ...
      val d = dr(v.drop(8+8))
        ...
        val decoded = param.toCodec.decode(vec)
        ...
        if (text.length % 254 == 0)
        	bodyDecode(gifH, vec.drop(12 * 8 + text.length + 8 * (text.length / (8 * 254))),GraphContrList,txt::TextList,ImList)
        else 
        	bodyDecode(gifH, vec.drop(12 * 8 + text.length + 8 * (text.length / (8 * 254)) + 8),GraphContrList,txt::TextList,ImList)
      }
      else{
