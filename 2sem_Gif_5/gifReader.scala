...
object Draw extends SimpleSwingApplication {
  val decodedpicture = new Decode("picture.gif");
  val width = decodedpicture.HeadOfGIF.sizes.width
  val height = decodedpicture.HeadOfGIF.sizes.height
  val scale = 10
  val el = decodedpicture.body.ImList.toArray
  var time = decodedpicture.body.GraphContrList.toArray
  if (time.isEmpty) {
    time = time :+ GraphicControl(0, 0, 1000, 0)
  }
  def top = new MainFrame {
    ...
    def tick(i: Int) ={
      ...
      { contents = new DataPanel(data, decodedpicture.HeadOfGIF.BG) {
...
case class GifHead(sizes: GIFSizes, BG: String, palitrpar: ColorsOfImage, GlobalPalitra: Array[BitVector])
...
case class ColorsOfImage(deep: Int, size: Int)
case class ImageParameters(left: Int, top: Int, pH: Int, pW: Int)

case class Images(imp: ImageParameters, lzwstart: Int, v: BitVector, palitraParam: ColorsOfImage, palitra: Array[BitVector]){
  ...
  private def decodeLZW (start: BitVector, tot: ArrayBuffer[List[Int]], s: Int, lastnum: Int): ArrayBuffer[List[Int]] = {
    if (start.take(s).reverseBitOrder.toInt(signed = false, LittleEndian) != maxnumber - 2 && tot.isEmpty)
      decodeLZW(start.drop(lzwstart), tot, lzwstart, 0)
    else if (start.take(s).reverseBitOrder.toInt(signed = false, LittleEndian) == maxnumber - 2 ) {
      library = library.take(maxnumber)
      decodeLZW(start.drop(s + lzwstart),
        tot += (start.drop(s).take(lzwstart).reverseBitOrder.toInt(signed = false,LittleEndian)::Nil),
        lzwstart, 0)
    }
    else  if (start.take(s).reverseBitOrder.toInt(signed = false,LittleEndian) == maxnumber - 1 || start.isEmpty) tot
    else{
      val n = start.take(s).reverseBitOrder.toInt(signed = false, LittleEndian)
      if (library.length > n) {
  ...
  private def colorArrayMake (l: List[Int], len: Int): Array[Array[String]] = {
  ...
  val decoded = colorArrayMake(predecodedimage, imp.pW)
}
...
class Decode(path : String) {
  ...
  private def pal(pa: BitVector): ColorsOfImage = {
    val d = pa.drop(1).take(3).toInt(signed = false,LittleEndian)
    val s = pa.drop(4).take(4).toInt(signed = false,LittleEndian)
    val par = ColorsOfImage(d + 1, math.pow(2, s + 1).toInt)
    par
  }
  private def colors (c: BitVector, p: ColorsOfImage): Array[BitVector] ={
  ...
  private def splitHead(bitvec: BitVector): GifHead = {
    ...
    if (!bitv(0)) {
      ...
      GifHead(Sizes, BG, ColorsOfImage(0, 0), null)
    }
    else {
  ...
  private def split(vec: BitVector): BitVector = {
    def cicle (vect: BitVector, lzwim: BitVector): BitVector = {
      val s = vect.take(8).toInt(signed = false, LittleEndian)
      if (s == 0) lzwim
  ...
  private def dr (v: BitVector): Int = {
    def cicle (vec: BitVector, d: Int): Int = {
      val s = vec.take(8).toInt(signed = false, LittleEndian)
      if (s == 0) d+8
      else cicle(vec.drop(8 + s * 8), d + 8 + s * 8)
    }
    cicle(v, 0)
  }
  private def decodeBody (gifH: GifHead, v: BitVector, GraphContrList: List[GraphicControl],
                          TextList: List[PlainText], ImList: List[Images]): GifBody = {
    if (hex"3b".bits == v.take(8)) GifBody(GraphContrList, TextList, ImList)
      ...
      if (vec.take(1) == true) {
        ...
        val pict = Images(decoded.require.value,
          vec.drop(8 + 8 * 3 * loc.size).take(8).toInt(signed = false,LittleEndian) + 1,
          split(vec.drop(8 + 8 * 3 * loc.size)), loc, colors(vec.drop(8), loc))
        if (pict.v.length % (254 * 8) == 0)
          decodeBody(gifH, vec.drop(16+8 + 8 * 3 * loc.size + pict.v.length + 8 * (pict.v.length / (8 * 254))),
            GraphContrList, TextList, pict :: ImList)
        else decodeBody(gifH, vec.drop(16+8 + 8 * 3 * loc.size + pict.v.length + 8 * (pict.v.length / (8 * 254)) + 8),
          GraphContrList, TextList, pict :: ImList)
      }
      else{
        val pict = Images(decoded.require.value,
          vec.drop(8).take(8).toInt(signed = false,LittleEndian) + 1,split(vec.drop(8)),
          gifH.palitrpar, gifH.GlobalPalitra)
        if (pict.v.length % (254 * 8) == 0)
          decodeBody(gifH, vec.drop(16+8 + pict.v.length + 8 * (pict.v.length.toInt / (8 * 254))),
            GraphContrList, TextList,  pict :: ImList)
        else
          decodeBody(gifH,vec.drop(16+8 + pict.v.length + 8 * (pict.v.length.toInt / (8 * 254)) + 8),
            GraphContrList, TextList, pict :: ImList)
      }
    }
    else{
      if (hex"f9".bits == v.drop(8).take(8) ){
        ...
        val vec = v.drop(16 + 8)
        val param = {
          (uint16L :: uint16L :: uint16L :: uint16L :: uint8L :: uint8L :: uint8L :: uint8L).as[TextParametrs]
        }
        ...
        for {i<- bytetext.indices}{
          t(i) = bytetext(i).toChar
        }
        val txt = PlainText(decoded.require.value,t)
        if (text.length % 254 == 0)
          decodeBody(gifH, vec.drop(12 * 8 + text.length + 8 * (text.length / (8 * 254))),
            GraphContrList,txt::TextList,ImList)
        else
          decodeBody(gifH, vec.drop(12 * 8 + text.length + 8 * (text.length / (8 * 254)) + 8),
            GraphContrList,txt::TextList, ImList)
      }
      else{
